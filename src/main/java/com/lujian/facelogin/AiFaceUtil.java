...
public class AiFaceUtil {
	...
	public static String Faceregistrtion(AipFace client, String groupId, Image image) {// 从指定的group中进行查找
		...
		String obj = res.get("result").toString();
		...
		if (obj == null || obj.equals("null")) {
			...
			for (int i = 0; i < user_list.length(); i++) {
				...
				String ob = job.get("score").toString();
				int score = 0;
				if (ob.equals("0")) {
					score = Integer.parseInt(ob);
				} else {
					double str = (double) job.get("score");
					score = (int) str;
				}
				// jsonId = (double) job.get("score"); // 得到每个对象中的id值
				if (score >= 90) {
